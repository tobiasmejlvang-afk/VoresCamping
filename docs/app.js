...
function blankRoute(){return {id:'',name:'',origin:'',destination:'',waypoints:'',distance:'',difficulty:'Let',description:''};}
...
function renderDetail(id){
  ...
  return shell(`${pageHeader(camp.status==='wishlist'?'Ønskebesøg':'Campingdagbog',camp.name,[camp.city,camp.country].filter(Boolean).join(', '),`<button class="btn secondary" data-nav="/rediger/${encodeURIComponent(camp.id)}">✎ Rediger</button><button class="btn danger" data-action="delete-camp" data-id="${attr(camp.id)}">🗑 Slet</button>`)}
    ...
    <div class="detail-grid"><div>
      ...
      <section class="detail-section card"><div class="section-head"><div><h2>Cykelruter</h2><p>Ruter åbnes med cykelvejledning i Google Maps.</p></div><button class="btn secondary small" data-nav="/rediger/${encodeURIComponent(camp.id)}">＋ Tilføj rute</button></div>${routes.length?routes.map(r=>`<article class="route-card card"><div class="route-head"><div><h3 style="margin:0">🚲 ${esc(r.name)}</h3><div class="route-meta">${r.distance?`<span class="pill">${esc(r.distance)}</span>`:''}<span class="pill">${esc(r.difficulty||'Ikke angivet')}</span></div></div><a class="btn primary small" target="_blank" rel="noopener" href="${attr(googleDirectionsUrl(r))}">Start rute ↗</a></div>${r.description?`<p>${esc(r.description)}</p>`:''}${r.origin&&r.destination?`<iframe class="route-map" title="Cykelrute i Google Maps" loading="lazy" referrerpolicy="no-referrer-when-downgrade" src="${attr(googleRouteEmbedUrl(r))}"></iframe>`:''}</article>`).join(''):`<div class="empty"><div class="big">🚲</div><p>Ingen cykelruter endnu.</p></div>`}</section>
    </div><aside>
...
function routeEditorHtml(){
  ...
  ${routeDraft.origin&&routeDraft.destination?`<iframe class="route-map" title="Forhåndsvisning af cykelrute" loading="lazy" referrerpolicy="no-referrer-when-downgrade" src="${attr(googleRouteEmbedUrl(routeDraft))}"></iframe>`:''}
...
function renderForm(id,status='visited'){
  ...
  return shell(`${pageHeader(editing?'Rediger campingplads':'Ny campingplads',editing?activeDraft.name:(status==='wishlist'?'Tilføj ønskebesøg':'Tilføj campingplads'),'Alle oplysninger kan ændres senere. Appen gemmer først, når du trykker Gem.',`<button class="btn secondary" data-action="form-cancel">← Annuller</button>`)}
    <form id="camp-form">
      ...
      <section class="form-card card"><h2>Cykelruter med Google Maps</h2><p class="sub">Gem start, slut og eventuelle stop. Ruten åbnes som cykelvejledning i Google Maps.</p><div class="route-list" id="draft-route-list">${draftRoutes()}</div><div class="card" style="padding:15px;margin-top:13px"><h3 style="margin-top:0">${routeEditIndex>=0?'Rediger cykelrute':'Ny cykelrute'}</h3><div id="route-editor">${routeEditorHtml()}</div></div></section>
...
document.addEventListener('click',async(event)=>{
  ...
  if(action==='map-scope'){
    ...
    if(!routeDraft.name.trim()||!routeDraft.origin.trim()||!routeDraft.destination.trim()){showToast('Ruten skal have navn, startsted og slutsted.','danger');return;}
    const saved={...deepClone(routeDraft),id:routeDraft.id||uid('route')};
